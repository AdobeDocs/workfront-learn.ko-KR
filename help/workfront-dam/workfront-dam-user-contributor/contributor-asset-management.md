...
자동 버전 제어가 켜져 있으면 에셋이 원래 에셋을 포함하는 폴더에 로드된 경우에만 에셋의 버전이 조정됩니다. 두 자산의 파일 이름은 같아야 합니다. 자산이 다른 폴더에 로드되면 자산이 새 파일로 들어갑니다.
버전 제어를 설정하지 않으면 기존 파일과 이름이 같은 파일이 어떤 폴더에 배치되는지에 관계없이 새 파일로 업로드됩니다. 이로 인해 같은 폴더에 이름이 같은 두 개의 자산이 있을 수 있습니다.
