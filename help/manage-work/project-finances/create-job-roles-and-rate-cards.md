---
title: 작업 역할 및 등급 카드 만들기
description: 작업 역할 및 비율 카드를 만드는 방법과 프로젝트에서 비율 카드를 첨부하고 제거하는 방법에 대해 알아봅니다.
activity: use
team: Technical Marketing
feature: Work Management
type: Tutorial
role: User
level: Intermediate
jira: KT-20259
last-substantial-update: '2026-05-08T00:00:00.000Z'
recommendations: noDisplay,catalog
doc-type: video
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2:
  - id: d968a1bc-9a90-4926-a531-bcf272c32aad
  - id: a0dacc9f-0e23-495b-8e9f-a77c2e60b40c
subfeature_v2:
  - id: f0dd7b45-76b5-49d4-afe3-39f436b6fbd3
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
autotag-review: '2026-05-06T14:51:17.520Z'
source-git-commit: ee03983e89e8c99e2fbdc12b108f3fb33d6d8e72
workflow-type: tm+mt
source-wordcount: 205
ht-degree: 13%

---

# 작업 역할 및 등급 카드 만들기

이 비디오에서는 작업 역할 및 비율 카드를 만드는 방법과 프로젝트에서 비율 카드를 첨부하고 제거하는 방법에 대해 설명합니다.


>[!VIDEO](https://video.tv.adobe.com/v/3486517/?quality=12&learn=on&enablevpops=1)

## 주요 학습 사항

* **작업 역할 구조 프로젝트 작업:** 작업 역할은 작업에 필요한 기술을 나타내며 사용자의 능력과 가용성에 따라 사용자에게 할당할 수 있습니다.
* **요금 카드 청구 요금 정의:** 요금 카드는 작업 역할에 대한 청구 요금을 나열하고 프로젝트 청구에 대한 고객과의 계약 역할을 합니다.
* **유연한 요금 관리:** 작업 역할과 요금 카드를 사용하면 유효 날짜가 포함된 비용 및 청구 요금을 설정할 수 있으므로 시간 기반 가격 변경을 지원합니다.
* **비율 카드 재정의:** 비율 카드가 프로젝트에 첨부된 경우 해당 비율은 작업 역할 비율보다 우선하지만 프로젝트별 재정의는 영향을 받지 않습니다.
* **쉬운 첨부 파일 및 업데이트:** 요금 카드를 프로젝트에서 추가하거나 제거할 수 있으며 정확한 청구를 위해 기존 프로젝트에 업데이트를 푸시할 수 있습니다.

## 이 주제와 관련된 추천 튜토리얼

* [성과 지표 이해](/help/manage-work/project-finances/understand-performance-metrics.md)
* [작업 매출 및 비용 기본값 설정](/help/manage-work/project-finances/set-up-task-revenue-and-cost-defaults.md)
* [환율 설정](/help/manage-work/project-finances/set-up-exchange-rates.md)
* [경비 유형 설정](/help/manage-work/project-finances/set-up-expense-types.md)
* [재무 액세스 이해](/help/manage-work/project-finances/understand-financial-access.md)
